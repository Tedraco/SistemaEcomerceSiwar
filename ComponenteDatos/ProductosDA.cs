...
namespace ComponenteDatos
{
    public class ProductosDA
    {
        ...
        public string InsertarProductos(Productos prod)
        {
            string rpta = "";
            try
            {
                ...
                cmdProducto.Connection = conn.conectarBD();
                {
                    ...
                    cmdProducto.Parameters.AddWithValue("@imagen", prod.Imagen);
                }
                int registros;
        ...
        public string ActualizarProductos(Productos prod)
        {
            string rpta = "";
            try
            {
                cmdProducto.CommandType = CommandType.StoredProcedure;
                cmdProducto.CommandText = "pa_Productos_actualizar";
                cmdProducto.Connection = conn.conectarBD();
                {
                    cmdProducto.Parameters.AddWithValue("@codproducto", prod.Codproducto);
                    cmdProducto.Parameters.AddWithValue("@desproducto", prod.Desproducto);
        ...
        public string EliminarProductos(Productos prod)
        {
            string rpta = "";
            try
            {
                cmdProducto.CommandType = CommandType.StoredProcedure;
                cmdProducto.CommandText = "pa_Productos_eliminar";
                cmdProducto.Connection = conn.conectarBD();
                {
                    cmdProducto.Parameters.AddWithValue("@codproducto", prod.Codproducto);
                }
        ...
        public Productos ListarporCategoria(string codcategoria)
        {
            ...
            try
            {
                ...
                if (lector.Read())
                {
                    ...
                    p.Preproducto = (decimal)(lector[3]);
        ...
        public List<Productos> UltimoCod()
        {
            SqlCommand sqlcmd = new SqlCommand("select count(codproducto),max (codproducto) from productos", conn.conectarBD());
            sqlcmd.CommandType = CommandType.Text;

            SqlDataReader PaTable = sqlcmd.ExecuteReader();
            List<Productos> Coleccion = new List<Productos>();
            while (PaTable.Read())
            {
                this.contador = Convert.ToString(PaTable.GetInt32(0));
                if (contador == "0")
                {
                    Coleccion.Add(new Productos(PaTable.GetInt32(0).ToString()));

                }
                else
                {
                    Coleccion.Add(new Productos(PaTable.GetString(1)));
